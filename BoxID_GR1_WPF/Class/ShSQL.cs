    ...
    public class ShSQL
    {
        ...
        public double sqlExecuteScalarDouble(string sql)
        {
            ...
            catch (Exception ex)
            {
                MessageBox.Show("SQL executeschalar method failed." + "\r\n" + ex.Message
                                , "Database Responce", MessageBoxButton.OK, MessageBoxImage.Warning);
                connection.Close();
                return 100;
            }
        }
        ...
        public bool sqlExecuteNonQuery(string sql, bool result_message_show)
        {
            try
            {
                ...
                if (response >= 1)
                {
                    if (result_message_show) { MessageBox.Show("Successful!", "Database Responce", MessageBoxButton.OK, MessageBoxImage.Information); }
                    connection.Close();
                    return true;
                }
                else
                {
                    MessageBox.Show("Not successful!", "Database Responce", MessageBoxButton.OK, MessageBoxImage.Warning);
                    connection.Close();
                    return false;
                }
            }
        ...
        public bool sqlMultipleInsertOverall(DataTable dt)
        {
            ...
            try
            {
                string sql = "INSERT INTO product_serial(serialno, lot, line, config, process, linepass, testtime, boxid, model) " +
                    "VALUES (:serialno, :lot, :line, :config, :process, :linepass, :testtime, :boxid, :model)";
                NpgsqlCommand command = new NpgsqlCommand(sql, connection);
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    command.Parameters[3].Value = dt.Rows[i]["config"].ToString();
                    command.Parameters[4].Value = dt.Rows[i]["process"].ToString();
                    command.Parameters[5].Value = dt.Rows[i]["linepass"].ToString();
                    command.Parameters[6].Value = DateTime.Parse(dt.Rows[i]["testtime"].ToString());
        ...
        public bool sqlMultipleInsert(DataTable dt)
        {
            int res1;
            bool res2 = false;
            connection = new NpgsqlConnection(conStringBoxidDb);
            ...
            try
            {
                string sql_del = "DELETE from product_serial where boxid = '" + dt.Rows[0]["boxid"].ToString() + "'";
                //sqlExecuteScalarString(sql_del);
                NpgsqlCommand command1 = new NpgsqlCommand(sql_del, connection);
