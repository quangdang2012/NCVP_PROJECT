...
namespace BoxIdDb
{
    public class ShSQL
    {
        ...
        public bool sqlMultipleInsertOverall(DataTable dt)
        {
            ...
            try
            {
                string sql = "INSERT INTO product_serial(serialno, lot, line, config, stationid, linepass, testtime, boxid, model) " +
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    command.Parameters[1].Value = dt.Rows[i]["model"].ToString();
                    command.Parameters[2].Value = dt.Rows[i]["lot"].ToString();
                    command.Parameters[3].Value = dt.Rows[i]["line"].ToString();
                    command.Parameters[4].Value = dt.Rows[i]["config"].ToString();
                    command.Parameters[5].Value = dt.Rows[i]["process"].ToString();
                    command.Parameters[6].Value = dt.Rows[i]["linepass"].ToString();
                    command.Parameters[7].Value = DateTime.Parse(dt.Rows[i]["testtime"].ToString());
                    command.Parameters[8].Value = dt.Rows[i]["boxid"].ToString();
