...
namespace BoxIdDb
{
    public class ShSQL
    {
        ...
        static string conStringBoxidDb = @"Server=192.168.145.12;Port=5432;User Id=mes;Password=dbuser;Database=mesdb; CommandTimeout=100; Timeout=100;";
        static string conStringTesterDb = @"Server=192.168.145.12;Port=5432;User Id=mes;Password=dbuser;Database=mesdb; CommandTimeout=100; Timeout=100;";
        ...
        public double sqlExecuteScalarDouble(string sql)
        {
            ...
            {
                MessageBox.Show("SQL executeschalar method failed." + "\r\n" + ex.Message
                                , "Database Responce", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                connection.Close();
                return 100;
            }
        }
        public int sqlExecuteScalarInt(string sql)
        {
            int response;
            try
            {
        ...
        public bool sqlMultipleInsertOverall(DataTable dt)
        {
            ...
            try
            {
                string sql = "INSERT INTO t_product_serial(boxid, serialno, line, lot, thurst, thurst_mc, noise, noise_mc, model) VALUES(:boxid, :serialno, :line, :lot, :thurst, :thurst_mc, :noise, :noise_mc, :model)";
                NpgsqlCommand command = new NpgsqlCommand(sql, connection);
                ...
                command.Parameters.Add(new NpgsqlParameter("thurst", NpgsqlTypes.NpgsqlDbType.Varchar));
                command.Parameters.Add(new NpgsqlParameter("thurst_mc", NpgsqlTypes.NpgsqlDbType.Varchar));
                command.Parameters.Add(new NpgsqlParameter("noise", NpgsqlTypes.NpgsqlDbType.Varchar));
                command.Parameters.Add(new NpgsqlParameter("noise_mc", NpgsqlTypes.NpgsqlDbType.Varchar));
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    command.Parameters[2].Value = dt.Rows[i]["line"].ToString();
                    command.Parameters[3].Value = dt.Rows[i]["thurst"].ToString();
                    command.Parameters[4].Value = dt.Rows[i]["thurst_mc"].ToString();
                    command.Parameters[5].Value = dt.Rows[i]["noise"].ToString();
                    command.Parameters[6].Value = dt.Rows[i]["noise_mc"].ToString();
                    command.Parameters[7].Value = dt.Rows[i]["boxid"].ToString();
                    command.Parameters[8].Value = dt.Rows[i]["model"].ToString();
        ...
        public bool sqlMultipleDelete(DataTable dt)
        {
            int res1;
            bool res2 = false;
            connection = new NpgsqlConnection(conStringBoxidDb);
            ...
            try
            {
                string sql = "DELETE FROM t_product_serial WHERE serialno = :serialno";
                NpgsqlCommand command = new NpgsqlCommand(sql, connection);
                ...
                {
                    transaction.Commit();
                    MessageBox.Show("Delete successful!", "Database Responce", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    connection.Close();
                    return true;
                }
                else
