...
namespace BoxIdDb
{
    public partial class frmHistory : Form
    {
        ...
        private void defineDatatable(ref DataTable dt)
        {
            ...
            dt.Columns.Add("ShipTo", Type.GetType("System.String"));
        ...
        private void txtSerial_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    string sql = "select boxid, serno, model, ship_date, status, shipto, invoice from t_shipment_record where serno = '" + serno + "'";
                    ...
                    if (dt1.Rows.Count != 0)
                    {
                        ...
                        string shipto = dt1.Rows[0][5].ToString();
                        string invoice = dt1.Rows[0][6].ToString();

                        newrow["boxid"] = boxid;
                        newrow["model"] = model;
                        newrow["shipdate"] = ship_date;
                        newrow["status"] = status;
                        newrow["shipto"] = shipto;
