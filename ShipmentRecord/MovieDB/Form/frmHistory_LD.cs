...
{
    public partial class frmHistory_LD : Form
    {
        DataTable dtHistory_LD;
        public frmHistory_LD()
        {
            InitializeComponent();
        }
        ...
        private void frmHistory_LD_Load(object sender, EventArgs e)
        {
            dtHistory_LD = new DataTable();
            defineDatatable(ref dtHistory_LD);
        }

        private void updateDataGridViews(DataTable dt, ref DataGridView dgv1)
        {
            dgvHistory_LD.DataSource = dt;
        ...
        private void txtSerial_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    string sql = "select serno, config, ship_date, status from shipmentld4 where serno = '" + serno + "'";
                    ...
                    DataRow newrow = dtHistory_LD.NewRow();
                    newrow["serial"] = serno;
                    ...
                    dtHistory_LD.Rows.Add(newrow);

                    // ƒf[ƒ^ƒOƒŠƒbƒgƒrƒ…[‚ÌXV
                    updateDataGridViews(dtHistory_LD, ref dgvHistory_LD);
                }
        ...
        {
            DataTable dt1 = new DataTable();
            dt1 = (DataTable)dgvHistory_LD.DataSource;
